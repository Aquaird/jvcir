  ...
  def forward(self, input, target_):
    ...
    for t in range(batchSize):
      ...
      if s < ref.eps and s > - ref.eps: #Sup data
        ...
        output += (self.regWeight * loss).cpu()
      else:
  ...
  def backward(self, grad_output):
    ...
    for t in range(batchSize):
      ...
      if s < ref.eps and s > - ref.eps:
        grad_input[t] += (grad_output[0] * self.regWeight / batchSize * 2 / ref.nJoints * (input[t] - z[t])).float().cpu()
      else:
        xy[t] = 2.0 * xy[t] / ref.outputRes - 1
        for g in range(len(self.skeletonRef)):
          E, num = 0, 0
          ...
          for j in range(N):
            if l[j] > 0:
              id1, id2 = self.skeletonRef[g][j]
              grad_input[t][id1] += (2 * self.varWeight * self.skeletonWeight[g][j] ** 2 / num * (l[j] - E) / l[j] * (input[t, id1] - input[t, id2]) / batchSize).float()
              grad_input[t][id2] += (2 * self.varWeight * self.skeletonWeight[g][j] ** 2 / num * (l[j] - E) / l[j] * (input[t, id2] - input[t, id1]) / batchSize).float()
